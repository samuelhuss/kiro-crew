/**
 * Unit tests for FileGraphRepository — the JSON-backed SHARED graph store.
 * Mirrors the previous Kuzu graph-repo tests: proves a graph written by one
 * instance is visible to another instance on the same dir (what lets the graph
 * and migration agents share it) — without an exclusive database lock.
 */

import { FileGraphRepository } from '../../repositories/graph/file-graph.repository.js';
...
describe('FileGraphRepository', () => {
  ...
  beforeEach(() => {
    tmpDir = mkdtempSync(join(tmpdir(), 'file-graph-'));
  });
  ...
  it('should save and read back a graph', async () => {
    const repo = new FileGraphRepository(tmpDir);
    await repo.init();
  ...
  it('should share the graph across two separate instances (no lock)', async () => {
    const writer = new FileGraphRepository(tmpDir);
    await writer.saveGraph(makeGraph());

    const reader = new FileGraphRepository(tmpDir);
    await reader.init();
